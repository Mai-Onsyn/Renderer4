...
export namespace Shader {
    ...
    _m256_Result fragmentShader_avx2(const _m256_Fragment& fragment, const Texture* texture, const Uniform* uniform, const Int32 count) {
        ...
        FColor ambient = FColor(uniform->ambient);
        ...
        for (const auto& light : uniform->lights) {
            ...
            // __m256 Specular_r = _mm256_set1_ps(0);
            // __m256 Specular_g = _mm256_set1_ps(0);
            // __m256 Specular_b = _mm256_set1_ps(0);
        ...
        // __m256 clamped_r = _mm256_min_ps(_mm256_max_ps(sum_r, _mm256_set1_ps(0.0f)), _mm256_set1_ps(1.0f));
        // __m256 clamped_g = _mm256_min_ps(_mm256_max_ps(sum_g, _mm256_set1_ps(0.0f)), _mm256_set1_ps(1.0f));
        // __m256 clamped_b = _mm256_min_ps(_mm256_max_ps(sum_b, _mm256_set1_ps(0.0f)), _mm256_set1_ps(1.0f));

        // __m256i color_r = _mm256_cvttps_epi32(_mm256_mul_ps(clamped_r, _mm256_set1_ps(255.0f)));
        // __m256i color_g = _mm256_cvttps_epi32(_mm256_mul_ps(clamped_g, _mm256_set1_ps(255.0f)));
        // __m256i color_b = _mm256_cvttps_epi32(_mm256_mul_ps(clamped_b, _mm256_set1_ps(255.0f)));
    ...
    _m256_Result fragmentShader_avx2_NoLight(const _m256_Fragment& fragment) {
        ...
        //
        // Vec8f va = (T_diff_r * (uniform->ambient.r * Ka.r));
        // Vec8f vb = (T_diff_g * (uniform->ambient.g * Ka.g));
        // Vec8f vc = (T_diff_b * (uniform->ambient.b * Ka.b));
        // va.store(stackI_amb_r);
        // vb.store(stackI_amb_g);
        // vc.store(stackI_amb_b);
        // va.store(stackSum_r);
        // vb.store(stackSum_g);
        //
        ...
        // Vec8f lenInv = Vec8f(1.0f) / (vx_nn * vx_nn + vy_nn * vy_nn + vz_nn * vz_nn).sqrt();
        // (vx_nn * lenInv).store(stackVx);
        // (vy_nn * lenInv).store(stackVy);
        // (vz_nn * lenInv).store(stackVz);
        //
        // // memcpy(stackSum_r, stackI_amb_r, 32);
        // // memcpy(stackSum_g, stackI_amb_g, 32);
        // // memcpy(stackSum_b, stackI_amb_b, 32);
        // for (const auto& light : uniform->lights) {
        ...
        //     Vec8f lenInv = Vec8f(1) / (H_x_nn * H_x_nn + H_y_nn * H_y_nn + H_z_nn * H_z_nn).sqrt();
        //     Vec8f H_x = H_x_nn * lenInv;
        //     Vec8f H_y = H_y_nn * lenInv;
        //     Vec8f H_z = H_z_nn * lenInv;
        //
